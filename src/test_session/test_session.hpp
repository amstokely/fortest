...
class TestSession {
    ...
    std::unordered_map<std::string, std::unique_ptr<TestSuite<Logger>>> m_suites;
...
public:
    ...
    void add_fixture(const std::string &suite_name, const Fixture &fixture) {
        if (fixture.get_scope() != "test" && fixture.get_scope() != "session") {
            throw std::runtime_error(
                "Fixture for suite '" + suite_name +
                "' must have test or session scope"
            );
        }
        auto it = m_suites.find(suite_name);
        if (it == m_suites.end()) {
            throw std::runtime_error(
                "Suite '" + suite_name + "' does not exist in session '"
            );
        }
        it->second->add_fixture(fixture);
    }

    void add_test(const std::string &suite_name,
    ...
    void run(const std::shared_ptr<Logger> &logger) {
        ...
        for (auto &[name, suite] : m_suites) {
            ...
            suite->add_fixture(Fixture(nullptr, nullptr, m_session_fixture ? m_session_fixture->get_args() : nullptr, "session"));
            suite->run(logger);
        }
